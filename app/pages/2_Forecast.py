...
st.info(
    ...
    "series. **17 of 27 folds converge cleanly, and every series has at least one clean "
    "fold** — the series-level count flags a series when *any* fold misses a threshold, "
    "which overstates the problem. Four of the ten failing folds miss on R-hat 1.011 "
    "against a 1.010 ceiling while mixing well (ESS 545-1,106), which is a threshold "
    "artifact rather than a sampler that failed. It is **not** a short-window effect: the "
    "shortest window has the best pass rate, 7 of 9. Two of the three series that beat the "
    "naive benchmark are still among the genuinely unconverged, so the layer is not ready "
    "to carry a headline."
)
